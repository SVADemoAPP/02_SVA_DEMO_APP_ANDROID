...
import com.hck.imagemap.view.Markun;
...
public class MainActivity extends Activity
{
    ...
    private void initView()
    {
        ...
        markun = (Markun) findViewById(MResource.getIdByName(getApplication(),
                "id", "tvChangeShow"));
    ...
    private void switchMap1(String mapPath)
    {
        ...
        try
        {
            ...
            fis = new FileInputStream(mapPath);
            bitmaps = BitmapFactory.decodeStream(fis);
            map.setMapBitmap(bitmaps);
            bmpWidth = currFloor.getImgWidth();
            bmpHeight = currFloor.getImgHeight();
            mapLoadfinish = true;
            thisOne[0] = 0;
        ...
        new Handler().postDelayed(new Runnable()
        {
            ...
            public void run()
            {
                if (!isBack)
                {
                    ...
                    {
                        wait = true;
                        if ((isRequestLocation || isDebugRequestLocation))
                        {
                            requestLocation();
                        } else
                        {
    ...
    {
        Map<String, String> params = new HashMap<String, String>();
        params.put("ip", /* load.getLocaIpOrMac() */"192.168.2.6");
        ...
        JsonObjectRequest newMissRequest = new JsonObjectRequest(
                ...
                new Response.Listener<JSONObject>()
                {
                    ...
                    public void onResponse(JSONObject jsonobj)
                    {
                        Log.d(TAG, "requestLocation:111" + jsonobj.toString());
                        ...
                        try
                        {
                            ...
                            if (mapLoadfinish && isMove)
                            {
                                if (autoSwitch)
                                {
                                    ...
                                    {
                                        markun.setVisibility(View.INVISIBLE);
                                    }
    ...
    /** 将空口结果转换为惯导结果并根据开关计算输出坐标 */
    private void path_INS(Location loc)
    {
        if ("ul".equals(currFloor.getCoordinate()))// 左上为原点
        {
            fi = (float) ((TargetDirection - 90) / 180 * PI);
    ...
    private void addPostion(Location loc)
    {
                ...
                && yy < bmpHeight - mapData.get(val).getY() * f.getScale())
        {
            ...
            {
                CircleShape postion = new CircleShape("NO" + postionNum,
                        Color.RED);
                postion.setValues(String.format("%.5f:%.5f:10", xx, yy));
                map.addShape(postion, bAnimation);
                RequestShape requestShape = new RequestShape("s", Color.BLUE,
